...
public class JasyptUtils {
    ...
    /**
     * {@link BasicTextEncryptor} 用于轻松执行文本的常规强度加密与解密，内部使用 {@link StandardPBEStringEncryptor} 加密器
     * 同时使用 PBEWithMD5AndDES 算法，这个类是线程安全的。使用很简单。
     * 1、创建 {@link BasicTextEncryptor}
     * 2、设置加解密密码，setPassword(String) 或者 setPasswordCharArray(char[])}</tt>)
     * 3、执行加密 encrypt(String)，执行解密 decrypt(String)
     *
     * @param secretKey
     * @param message
     * @param isEncrypt
     * @return
     */
    public static String textEncryptor(String secretKey, String message, boolean isEncrypt) {
        BasicTextEncryptor basicTextEncryptor = new BasicTextEncryptor();
        basicTextEncryptor.setPassword(secretKey);
        String result = isEncrypt ? basicTextEncryptor.encrypt(message) : basicTextEncryptor.decrypt(message);
        return result;
    }
